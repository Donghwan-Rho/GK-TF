# GK-TF
This project is based on `Cramming` (paper: https://arxiv.org/abs/2212.14034)

## Evaluation With 128-Padding
* In the original `Cramming` GitHub repository, when performing the `GLUE` task, padding is added to make the number of tokens in the input text a multiple of 8. In this section, the number of tokens in all sentences is consistently set to 128. 
* After accessing `cramming/config/GLUE_sane.yaml`, please comment out all tasks under `defaults/tasks` except for the task you want (e.g., `rte`).
* To evaluate, run
```
...
## Get outputs of sentences one by one 
* After accessing `cramming/config/GLUE_sane.yaml`, please comment out all tasks under `defaults/tasks` except for the task you want (e.g., `rte`).
* Run `python sentense_test_eval.py name={your_name}`
