...
class TorchEngineMinimal(torch.nn.Module):
    ...
    def __init__(self, model, cfg_train, cfg_impl, already_elapsed_time=0.0, setup=_default_setup, seq_length=128):
        ...
        print('TorchEngineMinimal')
        print(f'TorchEngineMinimal setup: {setup}')
        # print(f'cfg_train:\n{cfg_train}')
        ...
        if torch.distributed.is_initialized():
            ...
            # print('not if torch.distributed.is_initialized()')
            # 여기
            self.model = model
    ...
    def step(self, batch: dict[str, torch.Tensor]):
        ...
        # print(f'self.accumulated_samples: {self.accumulated_samples}')
    ...
    def load_checkpoint(self, cfg_arch, file, skip_optim_state=True):
        print(colored(f'load_checkpoint file: {file}', 'yellow'))
    ...
    def save_final_model(self, base_directory, identifier, tokenizer, cfg_arch, dryrun=False):
        ...
        if not dryrun:
            ...
            # print(f'self.retrieve_model_state_dict(): {self.retrieve_model_state_dict()}')
...
class CustomAdamW_variant(torch.optim.Optimizer):
    ...
    def step(self):
        ...
        with open ('custom_adamw_sqrt_input_max.txt', 'a') as f:
            f.write(f'{max(sqrt_input_maxs)}\n')
        with open ('custom_adamw_sqrt_input_min.txt', 'a') as f:
...
def _load_optimizer(model, cfg_train, cfg_impl, initial_time):
    # print(f'_load_optimizer cfg_train: {cfg_train}')
    # print(f'_load_optimizer cfg_impl: {cfg_impl}')
    # Filter some parameters
    grouped_parameters = group_parameters(model, cfg_train)
    # print(f'_load_optimizer grouped_parameters: {grouped_parameters}')

    # Select optimizer implementation
    if cfg_train.optim.type == "AdamW":
        print('if cfg_train.optim.type == "AdamW"')
        ...
        print('if cfg_train.optim.type == "SGD"')
    ...
    if torch.distributed.is_initialized() and cfg_impl.zero_redundancy_optimizer:
        ...
        print(f'_load_optimizer optimizer_args: {optimizer_args}')
        optimizer = optimizer_class(grouped_parameters, **optimizer_args) # opt class: adamW

    if cfg_train.optim_mod.name == "none": # 여기
    ...
    print(f'_load_optimizer optimizer: {optimizer}')
    print(f'_load_optimizer scheduler: {scheduler}')
