...
def prepare_pretraining_dataloader(dataset, tokenizer, cfg_train, cfg_impl):
    ...
    if cfg_train.objective.name == "masked-lm":
        print(f'if cfg_train.objective.name == "masked-lm":')
        # 여기
    ...
    if isinstance(dataset, torch.utils.data.IterableDataset):
        # 여기
        print(f'if isinstance(dataset, torch.utils.data.IterableDataset):')
        ...
        if torch.distributed.is_initialized(): # 실행 X
            dataset = split_dataset_by_node(dataset, rank=int(os.environ["RANK"]), world_size=int(os.environ["WORLD_SIZE"]))

        print(f'cfg_impl.shuffle_in_dataloader: {cfg_impl.shuffle_in_dataloader}')
        if cfg_impl.shuffle_in_dataloader: # 실행 X
            dataset = dataset.shuffle(seed=42, buffer_size=256)
        else:
            # 여기
            print(f'not if cfg_impl.shuffle_in_dataloader')
