...
class Custom_CrossEntropyLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        # logits: 모델의 예측 (logits), 크기는 (N, C)
        # labels: 정답 레이블, 크기는 (N,)

        softmax_input_max = logits.max().item()
        with open ('softmax_input_max.txt', 'a') as f:
            f.write(f'{softmax_input_max}\n')
        softmax_input_min = logits.min().item()
...
class EmbeddingComponent_modified(torch.nn.Module):
    def __init__(self, cfg_embedding, norm, norm_eps):
        ...
        # print(colored(f'cfg_embedding: {self.cfg_embedding}', 'magenta'))
        # print(colored(f'cfg_embedding.get_input_range: {cfg_embedding.get_emb_input_range}', 'light_magenta'))
        if cfg_embedding.pos_embedding == "learned":
            self.pos_embedding = LearnablePositional(cfg_embedding.embedding_dim, cfg_embedding.max_seq_length)
        elif cfg_embedding.pos_embedding == "sinusoidal":
            self.pos_embedding = SinusoidalPositional(cfg_embedding.embedding_dim, cfg_embedding.max_seq_length)
            ...
            # 여기
            self.pos_embedding = ScaledSinosoidal(cfg_embedding.embedding_dim, cfg_embedding.max_seq_length)
            # print('elif cfg_embedding.pos_embedding == "scaled-sinusoidal"', self.pos_embedding)
        else:
            self.pos_embedding = None
        # print('self.pos_embedding', self.pos_embedding)
        self.dropout = torch.nn.Dropout(p=cfg_embedding.dropout_prob, inplace=INPLACE)
        if cfg_embedding.normalization: # True
            ...
            if norm in ["Approx_LayerNorm"]:
                ...
                self.norm = _get_norm_fn(norm)(cfg_embedding.embedding_dim, div_max=div_max, eps=norm_eps)
            else:
                self.norm = _get_norm_fn(norm)(cfg_embedding.embedding_dim, eps=norm_eps)
    ...
    def forward(self, input_ids):
        # print('\n============== Start embed_modified forward ==============')
        # print('input_ids', input_ids) # b, 128
        # print('input_ids', input_ids.shape) # b, 128
        # print('modified emb input_ids', input_ids.shape)
        # print('modified emb input_ids', input_ids.dtype)
        embeds = self.word_embedding(input_ids)
        ...
        # print('emb input_ids', input_ids.shape)
        # for i in range(1):
        #     print(f'self.word_embedding(input_ids)[{i}]', embeds[i])
        # print('self.word_embedding(input_ids)', embeds.shape)
        # print('self.word_embedding(input_ids)', embeds.dtype)
        # print('emb embeds', embeds.dtype)
        if self.pos_embedding is not None:
            # print('if self.pos_embedding is not None')
            embeds += self.pos_embedding(input_ids)
            # for i in range(1):
            #     print(f'self.pos_embedding(input_ids)[0][{i}]: {self.pos_embedding(input_ids)[0][i]}')
            # print(f'self.pos_embedding(input_ids): {self.pos_embedding(input_ids).shape}')
            # print(f'self.pos_embedding(input_ids): {self.pos_embedding(input_ids).dtype}')
            # embeds = embeds.to(dtype=torch.float64)
            # self.pos_embedding(input_ids): [1, max_seq_length, 768]
            # print('after positional embedding embeds', embeds.shape)
            # print('after positional embedding embeds', embeds.dtype)
            # print('embed forward self.pos_embedding(input_ids)', self.pos_embedding(input_ids))
        # self.norm = self.norm.to(dtype=torch.float64)
        if self.stabilize_low_precision:
            # Stabilize as in bnb StableEmbedding
            return self.dropout(self.norm(embeds.to(torch.get_default_dtype()))).to(embeds.dtype)
        else:
            # print('not if self.stabilize_low_precision')
            # print(f'self.norm(embeds): {self.norm(embeds).shape}')
            # print(f'self.norm(embeds): {self.norm(embeds).dtype}')
            # print(f'return self.dropout(self.norm(embeds)): {self.dropout(self.norm(embeds)).shape}')
            # print(f'return self.dropout(self.norm(embeds)): {self.dropout(self.norm(embeds)).dtype}')
            # Ordinary
            #######################################
            # print('============== End embed_modified forward ==============\n')        
            norm_inputs = embeds
            # print(f'\nembeds var input max/min: {torch.max(norm_inputs)}/{torch.min(norm_inputs)}')
            if self.cfg_embedding.get_emb_input_range:
                return self.dropout(self.norm(embeds)), norm_inputs
            else:
                return self.dropout(self.norm(embeds))
...
class Approx_LayerNorm(nn.Module):
    def __init__(self, normalized_shape, div_max, eps=1e-5):
        ...
        # print(f'self.div_max: {self.div_max}')        
        
    def forward(self, x):
        # print(f'self.div_max: {self.div_max}')
        # 평균과 분산을 계산합니다.
        # print(f'x max/min: {torch.max(x)}/{torch.min(x)}')
        mean = x.mean(dim=-1, keepdim=True)
        # print(f'mean max/min: {torch.max(mean)}/{torch.min(mean)}')
        # print(f'mean: {mean.shape}')
        var = ((x - mean) ** 2).mean(dim=-1, keepdim=True)
        # print(f'x - mean max/min: {torch.max(x - mean)}/{torch.min(x - mean)}')
        # print(f'x - mean: {(x-mean).shape}')
        # print(f'var max/min: {torch.max(var)}/{torch.min(var)}')
        # print(f'var+self.eps: {var+self.eps}')
        # print(f'var: {var.shape}')
        # 여기서 torch.sqrt()를 다항식으로 근사
        # print(f'torch.sqrt(var + self.eps): {torch.sqrt(var + self.eps)}')
        var_eps = var + self.eps
        var_eps = var_eps
        # print(f'var_eps: {var_eps.dtype}')
        # max_ = torch.max(var+self.eps).item()
        # min_ = torch.min(var+self.eps).item()
        # print(f'torch.max(var+self.eps).item(): {max_}')
        # print(f'torch.min(var+self.eps).item(): {min_}')
        # if torch.isnan(torch.max(var+self.eps)) or torch.isnan(torch.min(var+self.eps)):
        #     raise ValueError("NaN detected in the computation, terminating the program.")
        # print(f'var max/min: {max_/min_}')
        
        inverse_sqrt = Inverse_sqrt_approx((var_eps) / self.div_max) / self.sqrt
        inverse_sqrt = inverse_sqrt
        # print(f'approx inverse_sqrt max/min: {torch.max(inverse_sqrt)}/{torch.min(inverse_sqrt)}')
        # print(f'real 1/sqrt max/min: {torch.max((1/torch.sqrt(var_eps/ self.div_max)) / self.sqrt)}/{torch.min((1/torch.sqrt(var_eps / self.div_max)) / self.sqrt)}')
        # print(f'inverse sqrt: {1/torch.sqrt(var + self.eps)}')
        # print(f'inverse sqrt: {torch.sqrt(var + self.eps).dtype}')
        
        # Layer normalization 공식을 적용합니다.
        # x_normalized = (x - mean) / torch.sqrt(var + self.eps)
        x_normalized = (x - mean) * inverse_sqrt
        # print(f'x_normalized max/min: {torch.max(x_normalized)}/{torch.min(x_normalized)}')
...
class PoolingComponent(torch.nn.Module):
    def __init__(self, cfg_head, main_model_hidden_size):
        ...
        # print('PoolingComponent')
        # print('Pooling cfg_head', cfg_head)
        ...
        # print(f'self.pool_scheme: {self.pool_scheme}')
        
        # print(f'cfg_head: {cfg_head}')
        self.get_input_range = cfg_head.get_input_range
        self.get_grad = cfg_head.get_grad
        # print(f'self.get_grad: {self.get_grad}')

    def forward(self, hidden_states):
        # print(f'pooling hidden_states: {hidden_states.shape}')
        if self.get_grad:
            before_zero_indexing_hidden_states = hidden_states
            before_zero_indexing_hidden_states.retain_grad()
            # print('before_zero_indexing_hidden_states.retrain_grad()')
        ...
        if self.pool_scheme == "zero_index":
            ...
            # print(f'zero_index first_token_tensor = hidden_states[:, 0]: {first_token_tensor.shape}')
            # print(f'first_token_tensor = hidden_states[:, 0]: {first_token_tensor}')
        elif self.pool_scheme == "avg":
            first_token_tensor = hidden_states.mean(dim=1)
            # print(f'avg first_token_tensor = {first_token_tensor.shape}')
        elif self.pool_scheme == "max":
            first_token_tensor = hidden_states.max(dim=1)[0]
            # print('first_token_tensor = hidden_states.max(dim=1)[0]')
        elif self.pool_scheme == "lse":
            first_token_tensor = hidden_states.logsumexp(dim=1)
            # print('first_token_tensor = hidden_states.logsumexp(dim=1)')
        else:
        ...
        # Ordinary
        #######################################
        # print(f'self.dropout(pooled_output): {self.dropout(pooled_output).shape}')
        # print('======== End poolingcomponent ========\n')
...
class PoolingComponent_lora(torch.nn.Module):
    ...
    def forward(self, hidden_states):            
        ...
        if self.pool_scheme == "zero_index":
            ...
            # print(f'zero_index first_token_tensor = hidden_states[:, 0]: {first_token_tensor.shape}')           
            # print(f'zero_index first_token_tensor = hidden_states[:, 0]: {first_token_tensor.dtype}')           
            # print(f'first_token_tensor = hidden_states[:, 0]: {first_token_tensor}')
            

        dense_lora = self.lora_a(first_token_tensor)
        dense_out = self.lora_b(dense_lora)
        # print(f'dense_out: {dense_out.dtype}')
        pooled_output = self.activation(dense_out)
        # tanh output 범위 조사
        #######################################
        tanh_output_max = pooled_output.max().item()
        ...
        #######################################
        # print(f'self.dropout(pooled_output): {self.dropout(pooled_output).shape}')
        # print(f'self.dropout(pooled_output): {self.dropout(pooled_output).dtype}')
        # print('======== End poolingcomponent ========\n')
        if self.get_input_range:
            return self.dropout(pooled_output), dense_out
        else:
...
def _get_norm_fn(norm_name):
    # 여기에서 Approx_LayerNorm같은 것 불러오면 될 듯?
    # print(f'_get_norm_fn norm_name: {norm_name}')
...
def _get_nonlin_fn(nonlin_name, use_gating=True):
    # print('start _get_nonlin_fn')
    # print('nonlin_name', nonlin_name) # default: GELUglu
    if nonlin_name == 'GELU_poly_11glu':
        if 'glu' in nonlin_name.lower():
            # print('if "glu" in nonlin_name.lower()')
            nonlin_name = nonlin_name.split("glu")[0] # GELU
            # print('nonlin_name = nonlin_name.split("glu")[0]', nonlin_name)
            wrap_in_glu = use_gating # True
            # print('wrap_in_glu = use_gating', wrap_in_glu)
        ...
        # print('return: ', Tanh_poly_11())
        return Tanh_poly_11  
    else:
        # ex)SiLUglu -> 이름에서 glu 떼어내고 getattr을 통해 torch.nn.SiLU 불러옴
        # Approx_SiLU 이용하면 이름에서 glu 떼어내고 getattr 이용하면 될 듯?
        if "glu" in nonlin_name.lower():
            # 여기
            # print('if "glu" in nonlin_name.lower()')
        ...
        # print('nonlin_fn = getattr(torch.nn, nonlin_name)', nonlin_fn)
        try:
            # 여기
            # print('try:')
            # 이 아래 부분은 TypeError라 실행되지 않음
            if nonlin_name == "Approx_ReLU":
                nonlin_fn = Approx_ReLU
            else:
                nonlin_fn = partial(nonlin_fn, inplace=INPLACE)
            nonlin_fn()
            # print('try nonlin_fn()', nonlin_fn())
        except TypeError:
            if nonlin_name == "Approx_ReLU":
                nonlin_fn = Approx_ReLU
            else:
        ...
        if wrap_in_glu:
            # print('if wrap_in_glu:')
            # print(colored(f'nonlin_fn: {nonlin_fn}', 'yellow'))
            # print('return partial(GLU, nonlin_fn)', partial(GLU, nonlin_fn))
            return partial(GLU, nonlin_fn)
        else:
            # print('nonlin_fn', nonlin_fn)
            return nonlin_fn
...
class GLU(torch.nn.Module):
    ...
    def __init__(self, sub_activation):
        super().__init__()
        # 여기서 approx_SiLU같은 것 추가
        if sub_activation == 'GELU_poly_11':
            self.sub_activation = GLUE_poly_11()
        # elif sub_activation == torch.nn.GELU(approximate='none'):
        #     self.sub_activation = sub_activation()
        else:
            self.sub_activation = sub_activation()
        # print('start GLU')
        # print('self.sub_activation', self.sub_activation)

    def forward(self, inputs):
        # print('\n======== Start GLU ========')
        # print('inputs', inputs.shape) # 128 b 3072
        # print('inputs', inputs.dtype) # 128 b 3072
        x, gate = inputs.chunk(2, dim=-1)
        # print('x', x.shape) # 128 b 1536
        # print('gate', gate.shape) # 128 b 1536
        # print(f'self.sub_activation(gate) * x: {(self.sub_activation(gate) * x).shape}')
        # print(f'self.sub_activation(gate) * x: {(self.sub_activation(gate) * x).dtype}')
        # print('======== End GLU ========\n')
