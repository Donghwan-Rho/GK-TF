...
def _get_nonlin_fn_grad(nonlin_name, use_gating=True):
    # if 'ReLU' in nonlin_name:
    #     nonlin_name = "Approx_ReLUglu"
    # else:
    #     'nonlin_name' = 
...
class TransformerLayer_grad(torch.nn.Module):
    ...
    def forward(self, states, attention_mask: Optional[torch.Tensor] = None):
        ...
        # print(f'attenion input - self.after_ln_before_att: {self.after_ln_before_att.shape}')
        # print(f'attenion input - self.after_ln_before_att: {self.after_ln_before_att.dtype}')
        # print(f'attenion input - attention_mask: {attention_mask.shape}')
        # print(f'attenion input - attention_mask: {attention_mask.dtype}')
...
class ScriptableLM_grad(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # self.cfg.final_norm = "Approx_LayernNorm"
...
class PoolingComponent_grad(torch.nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.pool_scheme == "zero_index":
            ...
            # self.first_token_tensor = hidden_states.mean(dim=1)
            self.first_token_tensor.retain_grad()
            

        self.dense_out = self.dense(self.first_token_tensor) # Inputs of the activation ftn
