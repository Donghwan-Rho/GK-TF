    ...
    for task_name, task in tasks.items():
        ...
        if lora_rank > 0:          
            for layer in model.encoder.layers:
                # attention qkv
                print(f'original weight: {layer.attn.self_attention.query_key_value.weight.data[0][:10]}')
                qkv_lora = Linear(768, 768*3, r=lora_rank, lora_alpha=lora_alpha, lora_dropout=0.1, fan_in_fan_out=False, merge_weights=False)
                ...
                print(f'after lora weight: {layer.attn.self_attention.query_key_value.weight.data[0][:10]}')
                # # attention dense
                # att_dense_lora = Linear(768, 768, r=lora_rank, lora_alpha=lora_alpha, lora_dropout=0.1, fan_in_fan_out=False, merge_weights=False)
                # att_dense_lora.weight.data.copy_(layer.attn.dense.weight.data)
                # layer.attn.dense = att_dense_lora
                # # ffn dense_in
                # ffn_dense_in_lora = Linear(768, 3072, r=lora_rank, lora_alpha=lora_alpha, lora_dropout=0.1, fan_in_fan_out=False, merge_weights=False)
                # ffn_dense_in_lora.weight.data.copy_(layer.ffn.dense_in.weight.data)
                # layer.ffn.dense_in = ffn_dense_in_lora
                # # ffn dense_out
                # ffn_dense_out_lora = Linear(1536, 768, r=lora_rank, lora_alpha=lora_alpha, lora_dropout=0.1, fan_in_fan_out=False, merge_weights=False)
                # ffn_dense_out_lora.weight.data.copy_(layer.ffn.dense_out.weight.data)
                # layer.ffn.dense_out = ffn_dense_out_lora
        ...
        for epoch in range(cfg.eval.epochs):
            ...
            for step, batch in enumerate(task["trainloader"]):
                ...
                # print(f"optimizer.param_groups[0]['lr']: {model_engine.optimizer.param_groups[0]['lr']}")
        ...
        # save_file(model_state_dict, f'{cfg.name}_eval_{task_name}_128padding.safetensors')
        ...
        # model_state_dict_loaded = load_file(f'{cfg.name}_eval_{task_name}_128padding.safetensors')
