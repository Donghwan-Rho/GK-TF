...
def main_training_process(cfg, setup):
    ...
    print(colored(f'main_training_process cfg: {cfg}', 'blue'))
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            log.info(f'Trainable parameters: {name}: {param.dtype}')
    for name, param in model.named_parameters():
        if not param.requires_grad:
            log.info(f'Untrainable parameters: {name}: {param.dtype}')
    ...
    log.info('\nAfter Pretrain')
    for name, param in model.named_parameters():
        log.info(f'param: {name}, dtype: {param.dtype}')
    log.info('\n')
