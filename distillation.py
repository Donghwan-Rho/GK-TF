...
def main_training_process(cfg, setup):
    ...
    teacher_model = cramming.construct_model(cfg.arch, cfg.data.vocab_size)
    print(colored('Model', 'green'))
    
    # for name, param in teacher_model.named_parameters():
    #     print(name)
    '''
    original model이나 modified model이나 weight 이름은 같다.
    그러므로 weight를 복사하기는 쉬울 것이다.
    '''
    
    '''
    teacher model의 ckpt 불러오기
    teacher model(t)의 ckpt를 student model(s)로 복사
    
    distillation 작업 위한 att, ffn output 등 return하는 모델 생성
    (새로운 .py 파일 및 class 만들기)
    
    s, t를 이용한 loss 형성(새로운 train ftn)
    ...
    print('checkpoint_rendevous', checkpoint_rendevous)
    ...
    # print(os.path.join(os.getcwd(), '../../../model.pth'))
